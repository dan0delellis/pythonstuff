...
parser.add_argument('--input', dest="input", help="Source input video file. Source directory if --directory is used. Will exit with an error if target is not a file or directory.", type=str, default="")
...
parser.add_argument('--destination-directory', dest="destination", help="Destination directory for file output. Default is current working directory.", default="", type=str)
parser.add_argument('--directory', dest="directory", help="Process all files found in a target directory. If --input is omitted, will assume current working directory. If --input specifies a non-directory (either a file or an invalid path), program will exit with an error. Ignores --output flag. Output files will have REENCODED- prepended to the filename if --destination-directory is not specified.", default=False, action="store_true")
