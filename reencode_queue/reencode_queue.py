...
if args.skip_file == args.config:
    logger("The filename to indicate a directory should be skipped is the same as the name the filename to indicate a directory should be scanned. This isn't gonna work")
    exit(255)
#flow:
    #parse options
...
if not os.path.isdir(args.source):
    logger("source dir {} is not a valid path".format(args.source))
    exit(1)
...
for root, _, files in os.walk(args.source,followlinks=True):
    ...
    #Does this live in a hidden directory?
        ...
        if in_hidden_dir(config_file):
            logger(f"{config_file} present, but is in a hidden dir")
            continue
    #Are there any actionable files? Loop through list of actionable files : rename magic file to args.skip_file
        ...
        for in_file in files:
            ...
            if os.path.isfile(f"{out_file}"):
                ...
                continue
            logger("    This is where we'd test if it has a video stream")
            if not check_video_stream(file_path,args.fprobe_path):
                logger("    This file is not a video file. Next... ")
                continue
            logger(f"    Set log file location:")
            log_dir = os.path.dirname(f"{args.log_dir}")
            log_file = f"{log_dir}/{in_file}.log"
            logger(f"    Assumed log file: {log_file}. Checking to see if path must be created.")
            create_path_if_needed(log_file, make_dir_for_filepath=True)
            logger("    This is where we'd feed it to ffmpeg_front.pl and hope for the best")
